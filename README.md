This project is a fork of IMPRIMANTES_3D, maintained by [sudtek].

# IMPRIMANTES_3D

24 août 2024

Ce repository dédié aux imprimantes 3D va regrouper mes howto et tutoriels principalement liés à certaines imprimantes 3D que j'utilise / posséde.
J'utilise principalement des imprimantes 3D pour l'impression de prototypes réalisée sous Catia V5 ,malheureusement par defaut les imprimantes 3D ne sont pas correctement calibrés par les constructeurs.
C'est donc à l'utilisateur de regler son imprimante via des étapes de calibrages pour esperer obtenir une objet 3D précis et fidéles au STL.

(English translation:)
# 3D PRINTERS 

August 24, 2024 

This repository dedicated to 3D printers will gather my how-tos and tutorials mainly related to certain 3D printers that I use/own. 
I primarily use 3D printers for printing prototypes created with Catia V5. Unfortunately, by default, 3D printers are not properly calibrated by the manufacturers.
Therefore, it is up to the user to adjust their printer through calibration steps to obtain a 3D object.…
